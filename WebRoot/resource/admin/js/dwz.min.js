...
bar.animate({left:barleft},50,function(){
bar.hide();
sbar.show().css("left",-50).animate({left:0},20);
$(window).trigger(DWZ.eventType.resizeGrid);});});
$(op.collapse,sbar).click(function(){
var sbarwidth=sbar.cssv("left")+sbar.outerWidth();
if(bar.is(":hidden")){
$(op.toggleBut,bar).hide();
bar.show().animate({left:sbarwidth},50);
...
DWZ.ui.sbar=true;
sbar.animate({left:-25},20,function(){
bar.show();});
bar.animate({left:0},80,function(){
$(op.splitBar).show();
...
var spbar2=$(this);
setTimeout(function(){spbar2.show();},10);
...
function initLayout(){
var iContentW=$(window).width()-(DWZ.ui.sbar?$("#sidebar").width()+5:29);
...
var navMenuIndex=$tab.attr("navMenuIndex");
if(navMenuIndex) $('#navMenu>ul>li').eq(navMenuIndex).find('a').click();
...
this._init();
this._scrollCurrent();
this._getTabs().eq(this._currentIndex).attr("url",url).attr("navMenuIndex",$('#navMenu>ul>li.selected').index());}};(function($){
$.fn.extend({
...
$grid.prepend('<input id="'+sTarget+'" type="hidden" />');}
$("#"+sTarget,$grid).val($tr.attr("rel"));}});
$tr.dblclick(function(){$(".edit",navTab.getCurrentPanel()).click();});
});
...
dwzPageBreak({targetType:op.targetType,rel:op.rel,data:{orderField:orderField,orderDirection:}});});});},
